...
const filterByColor = (color: Colors) => {
...
describe('Deck', function () {
  ...
  it('should have 25 cards for each color', function () {
    expect({
      RED: deck.cards.filter(filterByColor(Colors.RED)).length,
      BLUE: deck.cards.filter(filterByColor(Colors.BLUE)).length,
      GREEN: deck.cards.filter(filterByColor(Colors.GREEN)).length,
      YELLOW: deck.cards.filter(filterByColor(Colors.YELLOW)).length,
    }).toEqual({
      RED: 25,
      BLUE: 25,
      GREEN: 25,
